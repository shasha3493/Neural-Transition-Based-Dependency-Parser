...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        # no. of tokens (e.g., the last word in the stack, first word in the buffer etc.) for which embeddings are concatenated for input
        self.n_features = n_features #36
        self.n_classes = n_classes # 3 - referring to tranistions (left arc, right arc, stack)
        self.dropout_prob = dropout_prob
        self.embed_size = embeddings.shape[1] # dim of each word embedding # 50
    ...
    def embedding_lookup(self, w):
        ...
        x = torch.index_select(self.embeddings, 0, w.flatten()).view(w.size(0), -1) # gather word embeddings for all the word indices in all batches and reshape (batch_size, n_features*slef.embed_size)
        return x
    ...
    def forward(self, w):
        ...
        @param w (Tensor): input tensor of tokens (batch_size, n_features) (it contains wodr indeices for every batch 
        ...
        x = self.embedding_lookup(w) # (batch_size, n_features*slef.embed_size)
...
if __name__ == "__main__":
    ...
    embeddings = np.zeros((100, 30), dtype=np.float32) # initializes embeddings of shape (100, 30) (i.e. V = 100, self.embed_size=30) to all 0s 
    model = ParserModel(embeddings) # initializes the ParserModel class
    
    '''
    python parser_model.py -e
    
    It does the sanity check for embedding look up function. Creates a batch of word indices of shape (4, 36) (i.e. (bs, self.embed_size)) 
    by sampling into word indices from the vocabulary. Since all the embeddings in a vocabulary is initialized to zero, it checks if embedding_lookup()
    returns all 0s embeddings for the batch of word indices
    
    python parser_model.py -f
    
    It does the sanity check for forward() of PraserModel class. Creates a batch of word indices of shape (4, 36) (i.e. (bs, self.embed_size)) 
    by sampling into word indices from the vocabulary. Passes it to the forward function and checks for the output shape of the logits (should be
    (bs, 3) - 3 for the three transition actions
